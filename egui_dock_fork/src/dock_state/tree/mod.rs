...
impl<Tab> Tree<Tab> {
    ...
    /// Splits the node to the right with an empty pane (no tabs). The empty pane fills with space
    /// and can receive tabs via `push_to_focused_leaf` or `push_to_first_leaf`.
    #[inline(always)]
    pub fn split_right_empty(&mut self, parent: NodeIndex, fraction: f32) -> [NodeIndex; 2] {
        self.split(parent, Split::Right, fraction, Node::leaf_empty())
    }
    ...
    ///
    /// Note: Empty leaves (0 tabs) are allowed for empty pane slots.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    pub fn split(
        ...
        assert!(new.is_leaf(), "new node must be a leaf (empty leaves allowed)");
    ...
    ///
    /// If the node is emptied after the tab is removed, the leaf is kept as empty
    /// (allowing empty panes that fill space).
    ///
