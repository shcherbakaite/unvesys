...
impl<'tree, Tab> DockArea<'tree, Tab> {
    ...
    fn tab_bar(
        ...
        let tabs_count = self.dock_state[surface_index][node_index].tabs_count();
        let style = fade_style.unwrap_or_else(|| self.style.as_ref().unwrap());
        // Empty panes: no tab bar, just empty space
        let tab_bar_height = if tabs_count == 0 {
            0.0
        } else {
        ...
        // Empty panes: skip tab bar rendering
        if tabs_count == 0 {
