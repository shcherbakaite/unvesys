...
impl<R> UiState<R> {
    /// Returns row indices in display order (for export). Empty if not yet validated.
    pub fn display_order_indices(&self) -> Vec<usize> {
    ...
    pub fn push_new_command<V: RowViewer<R>>(
        ...
        let restore = match command {
            ...
            Command::MoveRow { from, to } => {
                let new_pos = if to.0 > from.0 { to.0 - 1 } else { to.0 };
                vec![Command::MoveRow {
                    from: RowIdx(new_pos),
                    to: from,
                }]
    ...
    fn cmd_apply<V: RowViewer<R>>(
        ...
        match cmd {
            ...
            Command::MoveRow { from, to } => {
                self.cc_dirty = true;
                table.dirty_flag = true;
                table.move_row(from.0, to.0);
                let new_pos = if to.0 > from.0 { to.0 - 1 } else { to.0 };
                self.queue_select_rows([RowIdx(new_pos)]);
            }
