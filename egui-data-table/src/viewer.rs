...
pub trait RowViewer<R>: 'static {
    ...
    /// Return true to draw a horizontal separator line below this row (e.g. group boundary).
    fn row_separator_below(&mut self, _row: &R) -> bool {
        false
    }

    /// Return true to show "Toggle separator below" in row context menu.
    fn row_separator_editable(&mut self) -> bool {
        false
    }

    /// Toggle the separator below this row. Called when user selects the context menu item.
    fn toggle_separator_below(&mut self, _row: &mut R) {
        // Default: no-op
    }
}
