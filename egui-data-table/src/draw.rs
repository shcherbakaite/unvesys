...
impl<'a, R, V: RowViewer<R>> Renderer<'a, R, V> {
    ...
    fn show_body(
        ...
        let render_fn = |mut row: egui_extras::TableRow| {
            ...
            // Render row header - draggable for row reorder
            let (_, head_resp) = row.col(|ui| {
                // Calculate the position where values start.
                row_elem_start = ui.max_rect().right_top();

                let header_id = ui.id().with(("row_header", row_id.0));
                let resp = ui.dnd_drag_source(header_id, row_id.0, |ui| {
                ...
                // Visual feedback when hovering with a dragged row
                if let Some(hovered) = resp.dnd_hover_payload::<usize>() {
                    let pointer = ui.input(|i| i.pointer.interact_pos());
                    if let Some(ptr) = pointer {
                        let rect = resp.rect;
                        let stroke = Stroke::new(2.0, Color32::from_rgb(100, 150, 255));
                        if *hovered != row_id.0 {
                            if ptr.y < rect.center().y {
                ...
                // Detect drop - push MoveRow command
                if let Some(dragged_idx) = resp.dnd_release_payload::<usize>() {
                    let pointer = ui.input(|i| i.pointer.interact_pos());
                    let insert_at = if let Some(ptr) = pointer {
                        let rect = resp.rect;
                        if *dragged_idx == row_id.0 {
                            row_id.0
                        } else if ptr.y < rect.center().y {
                            row_id.0
                        } else {
                            row_id.0 + 1
                        }
                    ...
                    commands.push(Command::MoveRow {
                        from: RowIdx(*dragged_idx),
                        to: RowIdx(insert_at.min(table.len())),
                    });
                }
            });
            ...
            for (vis_col, col) in visible_cols.iter().enumerate() {
                ...
                let (rect, resp) = row.col(|ui| {
                    ...
                    // Draw row separator below (in last column to span full width)
                    if vis_col.0 == visible_cols.len().saturating_sub(1)
                        && viewer.row_separator_below(&table.rows[row_id.0])
                    {
                        ui.painter().hline(
                            body_max_rect.x_range(),
                            ui_max_rect.bottom(),
                            Stroke::new(1.5, visual.strong_text_color()),
                        );
                    }
                });
                ...
                (resp.clone() | head_resp.clone()).context_menu(|ui| {
                    ...
                    // Row separator toggle (only on first column to avoid duplicates)
                    if vis_col.0 == 0 && viewer.row_separator_editable() {
                        ui.separator();
                        let has_sep = viewer.row_separator_below(&table.rows[row_id.0]);
                        let label = if has_sep {
                            "Remove separator below"
                        } else {
                            "Insert separator below"
                        };
                        if ui.button(label).clicked() {
                            viewer.toggle_separator_below(&mut table.rows[row_id.0]);
                            ui.close_menu();
                        }
                    }
                });
