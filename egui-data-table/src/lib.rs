...
impl<R> DataTable<R> {
    ...
    /// Returns rows in the order they are displayed (respects sorting and reordering).
    /// When the table has been rendered and has UI state, uses the display order from cc_rows.
    /// Otherwise returns rows in storage order.
    pub fn rows_in_display_order(&self) -> Vec<R>
    where
        R: Clone,
    {
        if let Some(ui) = &self.ui {
            ui.display_order_indices()
                .into_iter()
    ...
    /// Move a row from one index to another. Invalidates UI cache.
